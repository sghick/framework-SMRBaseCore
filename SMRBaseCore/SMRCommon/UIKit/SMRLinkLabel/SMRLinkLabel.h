...
//! Project version number for SMRLinkLabel.
FOUNDATION_EXPORT double SMRLinkLabelVersionNumber;

//! Project version string for SMRLinkLabel.
FOUNDATION_EXPORT const unsigned char SMRLinkLabelVersionString[];

@class SMRLinkLabelLink;

/**
 Vertical alignment for text in a label whose bounds are larger than its text bounds
 */
typedef NS_ENUM(NSInteger, SMRLinkLabelVerticalAlignment) {
    SMRLinkLabelVerticalAlignmentCenter   = 0,
    SMRLinkLabelVerticalAlignmentTop      = 1,
    SMRLinkLabelVerticalAlignmentBottom   = 2,
};

/**
 Determines whether the text to which this attribute applies has a strikeout drawn through itself.
 */
extern NSString * const kSMRStrikeOutAttributeName;

/**
 The background fill color. Value must be a `CGColorRef`. Default value is `nil` (no fill).
 */
extern NSString * const kSMRBackgroundFillColorAttributeName;

/**
 The padding for the background fill. Value must be a `UIEdgeInsets`. Default value is `UIEdgeInsetsZero` (no padding).
 */
extern NSString * const kSMRBackgroundFillPaddingAttributeName;

/**
 The background stroke color. Value must be a `CGColorRef`. Default value is `nil` (no stroke).
 */
extern NSString * const kSMRBackgroundStrokeColorAttributeName;

/**
 The background stroke line width. Value must be an `NSNumber`. Default value is `1.0f`.
 */
extern NSString * const kSMRBackgroundLineWidthAttributeName;

/**
 The background corner radius. Value must be an `NSNumber`. Default value is `5.0f`.
 */
extern NSString * const kSMRBackgroundCornerRadiusAttributeName;

@protocol SMRLinkLabelDelegate;

// Override UILabel @property to accept both NSString and NSAttributedString
@protocol SMRLinkLabel <NSObject>
...
/**
 `SMRLinkLabel` is a drop-in replacement for `UILabel` that supports `NSAttributedString`, as well as automatically-detected and manually-added links to URLs, addresses, phone numbers, and dates.
 
 ## Differences Between `SMRLinkLabel` and `UILabel`
 
 For the most part, `SMRLinkLabel` behaves just like `UILabel`. The following are notable exceptions, in which `SMRLinkLabel` may act differently:
 ...
 `SMRLinkLabel`, like `UILabel`, conforms to `NSCoding`. However, if the build target is set to less than iOS 6.0, `linkAttributes` and `activeLinkAttributes` will not be encoded or decoded. This is due to an runtime exception thrown when attempting to copy non-object CoreText values in dictionaries.
 ...
 */
@interface SMRLinkLabel : UILabel <SMRLinkLabel, UIGestureRecognizerDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame
           colorForNormalLink:(UIColor *)normal
           colorForActiveLink:(UIColor *)active
         colorForInactiveLink:(UIColor *)inactive;

@property (strong, nonatomic, readonly) UIColor *colorForNormalLink;      // default:blue
@property (strong, nonatomic, readonly) UIColor *colorForActiveLink;      // default:red
@property (strong, nonatomic, readonly) UIColor *colorForInactiveLink;    // default:gray
...
/**
 ...
 @discussion A `SMRLinkLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) IBOutlet id <SMRLinkLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `SMRLinkLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) SMRLinkLabelVerticalAlignment verticalAlignment;
...
/**
 Adds a link. You can customize an individual link's appearance and accessibility value by creating your own @c SMRLinkLabelLink and passing it to this method. The other methods for adding links will use the label's default attributes.
 
 @warning Modifying the link's attribute dictionaries must be done before calling this method.
 
 @param link A @c SMRLinkLabelLink object.
 */
...
/**
 The `SMRLinkLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
@protocol SMRLinkLabelDelegate <NSObject>
 ...
 */
- (void)attributedLabel:(SMRLinkLabel *)label
...
/**
 ...
 @discussion This method is called if no other delegate method was called, which can occur by either now implementing the method in `SMRLinkLabelDelegate` corresponding to a particular link, or the link was added by passing an instance of a custom `NSTextCheckingResult` subclass into `-addLinkWithTextCheckingResult:`.
...
@interface SMRLinkLabelLink : NSObject <NSCoding>

typedef void (^SMRLinkLabelLinkBlock) (SMRLinkLabel *, SMRLinkLabelLink *);
...
/**
 ...
 @c SMRLinkLabelDelegate tap methods, which will not be called for this link.
 */
@property (nonatomic, copy) SMRLinkLabelLinkBlock linkTapBlock;

/**
 A block called when this link is long-pressed.
 If non-nil, long pressing on this link will call this block instead of the
 @c SMRLinkLabelDelegate long press methods, which will not be called for this link.
 */
@property (nonatomic, copy) SMRLinkLabelLinkBlock linkLongPressBlock;
