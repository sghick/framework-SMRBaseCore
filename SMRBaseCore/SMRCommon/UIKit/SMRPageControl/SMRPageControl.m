...
static NSString *kPTScrollViewContentOffset = @"contentOffset";

@interface SMRPageControl ()
...
@implementation SMRPageControl

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        [self p_configurePropertys];
    }
    return self;
}
...
- (CGSize)sizeForNumberOfPages:(NSInteger)pageCount {
    CGFloat width = (pageCount - 1)*(_pageIndicatorSize.width + _pageIndicatorSpaing) + _pageIndicatorSize.width + _contentInset.left +_contentInset.right;
    CGFloat height = _currentPageIndicatorSize.height + _contentInset.top + _contentInset.bottom;
    return CGSizeMake(width, height);
}

- (void)p_configurePropertys {
...
- (void)p_updateIndicatorViewsBehavior {
