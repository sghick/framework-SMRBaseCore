...
//  SMRBaseCoreDemo
//
//  Created by Tinswin on 2020/1/13.
...
#define kSMRLineBreakWordWrapTextWidthScalingFactor (M_PI / M_E)

static CGFloat const SMRFLOAT_MAX = 100000;

NSString * const kSMRStrikeOutAttributeName = @"SMRStrikeOutAttribute";
NSString * const kSMRBackgroundFillColorAttributeName = @"SMRBackgroundFillColor";
NSString * const kSMRBackgroundFillPaddingAttributeName = @"SMRBackgroundFillPadding";
NSString * const kSMRBackgroundStrokeColorAttributeName = @"SMRBackgroundStrokeColor";
NSString * const kSMRBackgroundLineWidthAttributeName = @"SMRBackgroundLineWidth";
NSString * const kSMRBackgroundCornerRadiusAttributeName = @"SMRBackgroundCornerRadius";

#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 60000
const NSTextAlignment SMRTextAlignmentLeft = NSTextAlignmentLeft;
const NSTextAlignment SMRTextAlignmentCenter = NSTextAlignmentCenter;
const NSTextAlignment SMRTextAlignmentRight = NSTextAlignmentRight;
const NSTextAlignment SMRTextAlignmentJustified = NSTextAlignmentJustified;
const NSTextAlignment SMRTextAlignmentNatural = NSTextAlignmentNatural;

const NSLineBreakMode SMRLineBreakByWordWrapping = NSLineBreakByWordWrapping;
const NSLineBreakMode SMRLineBreakByCharWrapping = NSLineBreakByCharWrapping;
const NSLineBreakMode SMRLineBreakByClipping = NSLineBreakByClipping;
const NSLineBreakMode SMRLineBreakByTruncatingHead = NSLineBreakByTruncatingHead;
const NSLineBreakMode SMRLineBreakByTruncatingMiddle = NSLineBreakByTruncatingMiddle;
const NSLineBreakMode SMRLineBreakByTruncatingTail = NSLineBreakByTruncatingTail;

typedef NSTextAlignment SMRTextAlignment;
typedef NSLineBreakMode SMRLineBreakMode;
#else
const UITextAlignment SMRTextAlignmentLeft = NSTextAlignmentLeft;
const UITextAlignment SMRTextAlignmentCenter = NSTextAlignmentCenter;
const UITextAlignment SMRTextAlignmentRight = NSTextAlignmentRight;
const UITextAlignment SMRTextAlignmentJustified = NSTextAlignmentJustified;
const UITextAlignment SMRTextAlignmentNatural = NSTextAlignmentNatural;

const UITextAlignment SMRLineBreakByWordWrapping = NSLineBreakByWordWrapping;
const UITextAlignment SMRLineBreakByCharWrapping = NSLineBreakByCharWrapping;
const UITextAlignment SMRLineBreakByClipping = NSLineBreakByClipping;
const UITextAlignment SMRLineBreakByTruncatingHead = NSLineBreakByTruncatingHead;
const UITextAlignment SMRLineBreakByTruncatingMiddle = NSLineBreakByTruncatingMiddle;
const UITextAlignment SMRLineBreakByTruncatingTail = NSLineBreakByTruncatingTail;

typedef UITextAlignment SMRTextAlignment;
typedef UILineBreakMode SMRLineBreakMode;
#endif


static inline CTTextAlignment CTTextAlignmentFromSMRTextAlignment(SMRTextAlignment alignment) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 60000
    switch (alignment) {
...
#else
    switch (alignment) {
        case UITextAlignmentLeft: return kCTTextAlignmentLeft;
        case UITextAlignmentCenter: return kCTTextAlignmentCenter;
        case UITextAlignmentRight: return kCTTextAlignmentRight;
        default: return kCTTextAlignmentNatural;
    }
#endif
}

static inline CTLineBreakMode CTLineBreakModeFromSMRLineBreakMode(SMRLineBreakMode lineBreakMode) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 60000
...
static inline CGFloat SMRFlushFactorForTextAlignment(NSTextAlignment textAlignment) {
    switch (textAlignment) {
        case SMRTextAlignmentCenter:
            return 0.5f;
        case SMRTextAlignmentRight:
            return 1.0f;
        case SMRTextAlignmentLeft:
        default:
            return 0.0f;
    }
}

static inline NSDictionary * NSAttributedStringAttributesFromLabel(SMRLinkLabel *label) {
    ...
    if ([NSMutableParagraphStyle class]) {
        ...
        CTTextAlignment alignment = CTTextAlignmentFromSMRTextAlignment(label.textAlignment);
        ...
        if (label.numberOfLines == 1) {
            lineBreakMode = CTLineBreakModeFromSMRLineBreakMode(label.lineBreakMode);
        }
...
static inline CGSize CTFramesetterSuggestFrameSizeForAttributedStringWithConstraints(CTFramesetterRef framesetter, NSAttributedString *attributedString, CGSize size, NSUInteger numberOfLines) {
    ...
    CGSize constraints = CGSizeMake(size.width, SMRFLOAT_MAX);

    if (numberOfLines == 1) {
        // If there is one line, the size that fits is the full width of the line
        constraints = CGSizeMake(SMRFLOAT_MAX, SMRFLOAT_MAX);
    } else if (numberOfLines > 0) {
        // If the line count of the label more than 1, limit the range to size to the number of lines that have been set
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathAddRect(path, NULL, CGRectMake(0.0f, 0.0f, constraints.width, SMRFLOAT_MAX));
...
@interface SMRAccessibilityElement : UIAccessibilityElement
@property (nonatomic, weak) UIView *superview;
@property (nonatomic, assign) CGRect boundingRect;
@end

@implementation SMRAccessibilityElement
...
@property (readwrite, nonatomic, strong) SMRLinkLabelLink *activeLink;
@property (readwrite, nonatomic, strong) NSArray *accessibilityElements;

- (void) longPressGestureDidFire:(UILongPressGestureRecognizer *)sender;
@end

@implementation SMRLinkLabel {
@private
...
- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame
            colorForNormalLink:[UIColor blueColor]
            colorForActiveLink:[UIColor redColor]
          colorForInactiveLink:[UIColor grayColor]];
}

- (instancetype)initWithFrame:(CGRect)frame colorForNormalLink:(UIColor *)normal colorForActiveLink:(UIColor *)active colorForInactiveLink:(UIColor *)inactive {
    self = [super initWithFrame:frame];
       if (!self) {
           return nil;
       }
       
       _colorForNormalLink = normal;
       _colorForActiveLink = active;
       _colorForInactiveLink = inactive;
...
- (void)commonInit {
    ...
    if ([NSMutableParagraphStyle class]) {
        [mutableLinkAttributes setObject:self.colorForNormalLink forKey:(NSString *)kCTForegroundColorAttributeName];
        [mutableActiveLinkAttributes setObject:self.colorForActiveLink forKey:(NSString *)kCTForegroundColorAttributeName];
        [mutableInactiveLinkAttributes setObject:self.colorForInactiveLink forKey:(NSString *)kCTForegroundColorAttributeName];
    } else {
        [mutableLinkAttributes setObject:(__bridge id)[self.colorForNormalLink CGColor] forKey:(NSString *)kCTForegroundColorAttributeName];
        [mutableActiveLinkAttributes setObject:(__bridge id)[self.colorForActiveLink CGColor] forKey:(NSString *)kCTForegroundColorAttributeName];
        [mutableInactiveLinkAttributes setObject:(__bridge id)[self.colorForInactiveLink CGColor] forKey:(NSString *)kCTForegroundColorAttributeName];
    }
...
- (void)addLink:(SMRLinkLabelLink *)link {
    [self addLinks:@[link]];
}
...
- (void)addLinks:(NSArray *)links {
    ...
    for (SMRLinkLabelLink *link in links) {
        if (link.attributes) {
...
- (SMRLinkLabelLink *)addLinkWithTextCheckingResult:(NSTextCheckingResult *)result
                                  ...
                                  attributes:(NSDictionary *)attributes
{
    ...
    for (NSTextCheckingResult *result in results) {
        ...
        SMRLinkLabelLink *link = [[SMRLinkLabelLink alloc] initWithAttributes:attributes
...
- (SMRLinkLabelLink *)addLinkToPhoneNumber:(NSString *)phoneNumber
                                       withRange:(NSRange)range
{
...
- (SMRLinkLabelLink *)addLinkToDate:(NSDate *)date
            withRange:(NSRange)range
{
...
- (SMRLinkLabelLink *)addLinkToTransitInformation:(NSDictionary *)components
                                              withRange:(NSRange)range
{
...
- (SMRLinkLabelLink *)linkAtPoint:(CGPoint)point {
    ...
    SMRLinkLabelLink *result = [self linkAtCharacterIndex:[self characterIndexAtPoint:point]];
...
- (SMRLinkLabelLink *)linkAtRadius:(const CGFloat)radius aroundPoint:(CGPoint)point {
...
- (SMRLinkLabelLink *)linkAtCharacterIndex:(CFIndex)idx {
...
- (CFIndex)characterIndexAtPoint:(CGPoint)p {
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        CGFloat flushFactor = SMRFlushFactorForTextAlignment(self.textAlignment);
                ...
                context:(CGContextRef)c
{
    ...
    CFArrayRef lines = CTFrameGetLines(frame);
    NSInteger numberOfLines = self.numberOfLines > 0 ? MIN(self.numberOfLines, CFArrayGetCount(lines)) : CFArrayGetCount(lines);
    BOOL truncateLastLine = (self.lineBreakMode == SMRLineBreakByTruncatingHead || self.lineBreakMode == SMRLineBreakByTruncatingMiddle || self.lineBreakMode == SMRLineBreakByTruncatingTail);
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        if (lineIndex == numberOfLines - 1 && truncateLastLine) {
            ...
            if (!(lastLineRange.length == 0 && lastLineRange.location == 0) && lastLineRange.location + lastLineRange.length < textRange.location + textRange.length) {
                ...
                SMRLineBreakMode lineBreakMode = self.lineBreakMode;

                // Multiple lines, only use UILineBreakModeTailTruncation
                if (numberOfLines != 1) {
                    lineBreakMode = SMRLineBreakByTruncatingTail;
                }

                switch (lineBreakMode) {
                    case SMRLineBreakByTruncatingHead:
                        truncationType = kCTLineTruncationStart;
                        break;
                    case SMRLineBreakByTruncatingMiddle:
                        truncationType = kCTLineTruncationMiddle;
                        truncationAttributePosition += (lastLineRange.length / 2);
                        break;
                    case SMRLineBreakByTruncatingTail:
                    default:
               ...
               context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            CGColorRef strokeColor = (__bridge CGColorRef)[attributes objectForKey:kSMRBackgroundStrokeColorAttributeName];
            CGColorRef fillColor = (__bridge CGColorRef)[attributes objectForKey:kSMRBackgroundFillColorAttributeName];
            UIEdgeInsets fillPadding = [[attributes objectForKey:kSMRBackgroundFillPaddingAttributeName] UIEdgeInsetsValue];
            CGFloat cornerRadius = [[attributes objectForKey:kSMRBackgroundCornerRadiusAttributeName] floatValue];
            CGFloat lineWidth = [[attributes objectForKey:kSMRBackgroundLineWidthAttributeName] floatValue];
           ...
           context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            NSDictionary *attributes = (__bridge NSDictionary *)CTRunGetAttributes((__bridge CTRunRef) glyphRun);
            BOOL strikeOut = [[attributes objectForKey:kSMRStrikeOutAttributeName] boolValue];
...
- (void)setActiveLink:(SMRLinkLabelLink *)activeLink {
     ...
     limitedToNumberOfLines:(NSInteger)numberOfLines
{
    ...
    if (textSize.height < bounds.size.height) {
        ...
        switch (self.verticalAlignment) {
            case SMRLinkLabelVerticalAlignmentCenter:
                yOffset = CGFloat_floor((bounds.size.height - textSize.height) / 2.0f);
                break;
            case SMRLinkLabelVerticalAlignmentBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
            case SMRLinkLabelVerticalAlignmentTop:
            default:
                break;
        }
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.adjustsFontSizeToFitWidth && self.numberOfLines > 0) {
        ...
        CGSize maxSize = (self.numberOfLines > 1) ? CGSizeMake(SMRFLOAT_MAX, SMRFLOAT_MAX) : CGSizeZero;

        CGFloat textWidth = [self sizeThatFits:maxSize].width;
        CGFloat availableWidth = self.frame.size.width * self.numberOfLines;
        if (self.numberOfLines > 1 && self.lineBreakMode == SMRLineBreakByWordWrapping) {
            textWidth *= kSMRLineBreakWordWrapTextWidthScalingFactor;
        }
...
- (NSArray *)accessibilityElements {
    if (!_accessibilityElements) {
        @synchronized(self) {
            ...
            for (SMRLinkLabelLink *link in self.linkModels) {
                ...
                if (accessibilityLabel) {
                    SMRAccessibilityElement *linkElement = [[SMRAccessibilityElement alloc] initWithAccessibilityContainer:self];
            ...
            SMRAccessibilityElement *baseElement = [[SMRAccessibilityElement alloc] initWithAccessibilityContainer:self];
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            SMRLinkLabelLink *link = [self linkAtPoint:touchPoint];
...
#pragma mark - SMRLinkLabelLink

@implementation SMRLinkLabelLink
...
- (instancetype)initWithAttributesFromLabel:(SMRLinkLabel*)label
                         textCheckingResult:(NSTextCheckingResult *)result {
