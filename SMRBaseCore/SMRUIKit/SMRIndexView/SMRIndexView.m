...
- (void)refreshIndexItems {
    ...
    self.indexItems = [self.dataSource sectionIndexTitlesForIndexView:self];
    [self getAllItemsSize];
    [self initialiseAllAttributes];
    [self resetPosition];
}
...
- (void)getAllItemsSize {
    ...
    for (NSString *item in self.indexItems) {
        CGSize currentItemSize = [self string:item sizeWithFont:self.font];
    ...
    for (NSString *item in self.indexItems) {
        CGSize currentItemSize = [self string:item sizeWithFont:self.selectedItemFont];
...
- (void) initialiseAllAttributes {
    ...
    for (NSString *item in self.indexItems) {
        ...
        if (self.itemsAligment == NSTextAlignmentCenter) {
            ...
            CGSize itemSize = [self string:item sizeWithFont:self.font];
...
- (void)drawIndex {
    ...
    for (NSCache *itemAttributes in self.itemsAtrributes) {
        ...
        if (singleItemTextLayer.fontSize != CTFontGetSize(currentFont)) {
            ...
            CTTextAlignment alignment = kCTTextAlignmentJustified;//kCTJustifiedTextAlignment;
...
- (void)drawLabel:(NSString *)label withFont:(UIFont *)font forSize:(CGSize)size
    ...
    CGSize newSize = [self string:label
                     sizeWithFont:font
    ...
    // draw text
    [self string:label drawInRect:rect
        withFont:font
   lineBreakMode:lineBreak
       alignment:alignment];
...
- (CGSize)string:(NSString *)string sizeWithFont:(UIFont *)font {
    ...
    return [string sizeWithAttributes:attribute];
}
...
- (CGSize)string:(NSString *)string sizeWithFont:(UIFont *)font constrainedToSize:(CGSize)size lineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    return [string boundingRectWithSize:size options:NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
}

- (void)string:(NSString *)string drawInRect:(CGRect)rect withFont:(UIFont *)font lineBreakMode:(NSLineBreakMode)lineBreakMode alignment:(NSTextAlignment)alignment {
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    paragraphStyle.lineBreakMode = lineBreakMode;
